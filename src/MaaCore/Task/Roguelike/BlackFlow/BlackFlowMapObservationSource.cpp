...
<div class="navigator"><div class="nav-line"><span class="nav-label">层数</span><div id="floor-tabs" class="floor-tabs"></div><div id="view-tabs" class="segment view-switch"><button class="view-button active" data-view="current" type="button">当前观测</button><button class="view-button" data-view="notebook" type="button">探索笔记</button></div></div><div class="nav-line"><button id="previous-step" class="nav-button" type="button" aria-label="上一步">←</button><label class="layer-toggle"><input id="show-checkpoints" type="checkbox">观测补充记录</label><select id="decision" class="step-select" aria-label="本层观测和规划步"></select><button id="next-step" class="nav-button" type="button" aria-label="下一步">→</button><span id="step-counter" class="nav-label"></span></div><div class="nav-line planning-only"><span class="nav-label">候选</span><select id="candidate" class="candidate-select" aria-label="查看候选路线"></select></div></div>
...
const markerTypeNames={resident:'流窜“居民”',savage:'流窜“居民”',fruit_cache:'藏果地',informant:'线人与线索',light_end:'光明之末',god_sorrow:'神明之殇',civilization_ashes:'文明之烬'};
...
const reasonCategoryNames={strategy_end:'终点与任务约束',resource_reserve:'加工品预留',preferred_goal:'优先目标',development:'探索收益',risk_avoidance:'风险规避',safety_fallback:'安全兜底',tie_break:'同分比较',battle_stage_observation:'战斗关卡识别',node_identity_resolved:'节点身份补充'};
...
function mapSection(item){const floor=Number(item?.floor)||0,generation=Number(item?.map_generation)||0,sectionGeneration=Number(item?.map_section_generation??generation),outerFloor=Number(item?.tree_hole_outer_floor)||0,remembrance=Boolean(item?.floor_four_remembrance),key=item?.map_section_key||`floor-${outerFloor||floor}-generation-${sectionGeneration}${outerFloor?'-tree-hole':''}${remembrance?'-remembrance':''}`,label=item?.map_section_label||(outerFloor?`${outerFloor} 层树洞`:`${remembrance?'追忆 ':''}${floor} 层`);return{floor,generation,remembrance,key,label}}
const mapSections=[...new Map(history.map(item=>{const section=mapSection(item);return[section.key,section]})).values()];
let mapSectionKey=history.length?mapSection(history.at(-1)).key:'',decisionIndex=Math.max(0,history.length-1),candidateIndex=-1,mapView='current',floorDecisionIndexes=[];
...
function refreshDecisionOptions(selectLatest=false){
 const decision=$('decision');decision.replaceChildren();const sectionIndexes=[];history.forEach((item,index)=>{if(mapSection(item).key===mapSectionKey)sectionIndexes.push(index)});
 floorDecisionIndexes=sectionIndexes.filter(index=>$('show-checkpoints').checked||!isObservationCheckpoint(history[index]));
 if(!floorDecisionIndexes.length)floorDecisionIndexes=sectionIndexes;
 let planningStep=0,checkpointStep=0;
 for(const index of floorDecisionIndexes){const item=history[index],checkpoint=isObservationCheckpoint(item),option=document.createElement('option');option.value=String(index);option.textContent=checkpoint?`补充 ${++checkpointStep} · ${prettyReasonCategory(item.reason_category)}`:`规划 ${++planningStep} · 行动力 ${item.action_points_before??'?'} · ${prettyReasonCategory(item.reason_category)}`;decision.append(option)}
 if(selectLatest||!floorDecisionIndexes.includes(decisionIndex))decisionIndex=floorDecisionIndexes.length?floorDecisionIndexes.at(-1):Math.max(0,history.length-1);decision.value=String(decisionIndex);refreshStepControls()
}
...
$('show-checkpoints').addEventListener('change',()=>{refreshDecisionOptions();candidateIndex=-1;renderDecision()});
    ...
    std::string* error)
{
    ...
    next.floor = floor;
    if (floor == perception::TreeHoleFloor) {
        next.tree_hole_effect = request.tree_hole_effect;
    }
...
bool BlackFlowMapObservationSource::persist_diagnostics(const DiagnosticArtifactRequest& request, std::string* error)
{
    ...
    try {
        ...
        const std::string floor_directory_name =
            diagnostic_floor_directory(floor, request.snapshot.get("tree_hole_outer_floor", 0));
        ...
        snapshot["recognition_tree_hole_effect"] = m_last_result.tree_hole_effect;
        snapshot["recognition_tree_hole_color"] = m_last_result.tree_hole_color;
