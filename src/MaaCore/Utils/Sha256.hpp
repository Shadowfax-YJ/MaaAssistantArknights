...
inline std::string sha256(std::string_view input)
{
    ...
    for (size_t offset = 0; offset < message.size(); offset += 64U) {
        ...
        for (size_t index = 16U; index < words.size(); ++index) {
            const auto sigma0 = rotate_right(words[index - 15U], 7U) ^ rotate_right(words[index - 15U], 18U) ^
                                (words[index - 15U] >> 3U);
            const auto sigma1 = rotate_right(words[index - 2U], 17U) ^ rotate_right(words[index - 2U], 19U) ^
                                (words[index - 2U] >> 10U);
