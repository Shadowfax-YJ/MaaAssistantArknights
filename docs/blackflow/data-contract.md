...
与目标楼层不同，且不枚举 `evidence` 值。独立验证器及原始契约 schema 1 支持既有字段和目录，
无需更新固定 bundle、两个业务插件或通用宿主。旧包继续兼容；未知契约仍保留原件并待适配。
新运行减少错误事件与落点记录，不重算旧冻结报告，不失效原图 OCR 缓存或已审核的原输入。
按新输入生成新事实；未采到的楼层不作为阴性，实托邦/小八界及其他专题分子分母口径不变。

验证：Core 完整编译、351 个 C++ 用例（5812 个断言）、62 个树洞返回及弹窗归属场景、
49 个页面身份/部署/超载回放均通过；原始误判截图已通过当前生产 OCR 与分类函数复测。
analysis 的跨楼层弹窗标准化回归通过，更新发布测试 33 项中 25 项通过、8 项环境条件测试跳过。
