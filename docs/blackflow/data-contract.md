...
用户授权对证据充分的历史错误进行持久修订。`tools/ReviseBlackFlowRunArchive.py` 根据固定源 SHA256、
...
本次全量审核覆盖 1,532 个归档，生成 201 包的 210 次未知战斗修正及 2 包的回图身份修正；
此前双黑诞样本另有独立修订。169 次其余黑诞进场均有原图标题证据。
