...
4. 在某个击杀数关闭持续技能
   ...
   `skill_stop_plan` 按地图坐标定位已部署干员。条件命中后会直接点击技能按钮一次；每条指令只执行一次，不改变该干员后续的自动开技能设置。
...
5. 让场内固定装置自动释放技能
   ...
   对于具有干员式主动技能、且开局就固定在地图上的装置，可以将它们注册为虚拟的自动开技能单位。MAA 会从战斗开始起检查这些坐标，在技能就绪时点击装置并释放技能；每个坐标也会被视为已占用，不再用于部署干员。`name` 仅用于日志与区分虚拟单位，`locations` 使用 MAA 地图坐标。

   ```json5
   "virtual_auto_skill_devices": [
       {
           "name": "丰饶树冢",
           "locations": [
               [1, 5],
       ...
       {
           "name": "其他固定装置",
           "skill_times": 1, // 可选；成功开技次数上限，省略或填 0 表示持续自动开技
           "locations": [
               [6, 2]
           ]
       }
   ...
   如果必须等某个装置至少成功开技一次后才能开始部署，可额外填写其坐标。该坐标必须已经出现在 `virtual_auto_skill_devices` 中：

   ```json5
   "deploy_after_virtual_auto_skill": [6, 2]
   ```
...
`resource/roguelike/JieGarden/coppers.json` 描述了界园肉鸽通宝的配置和交换策略

```json5
{
    "theme": "JieGarden",                                    //肉鸽主题名（这里是界园）
    "addons": {                                              //通宝附加属性说明（仅作注释，不影响程序运行）
...
2. **交换钱盒通宝**：当新拾取的通宝需要与钱盒中的通宝交换时：
   - 识别钱盒中所有通宝的类型、名称和是否已投出状态
   - 找到钱盒中 `discard_priority` 最高（最不重要）的通宝
   - 如果新通宝的 `discard_priority` 比最不重要的现有通宝更低，则放弃交换
   - 否则将最不重要的通宝替换为新通宝

3. **已投出通宝的特殊处理**：
   - 对于标记了 `cast_discard_priority` 的通宝，在已投出状态时使用该优先级而非 `discard_priority`
   - 这通常适用于一些投出后效果发生变化的通宝（如变为大炎通宝的通宝）
...
1. 通过模板匹配识别通宝的类型图标（厉/衡/花）
2. 基于类型图标的位置，使用 ROI 偏移进行 OCR 识别通宝名称
3. 识别通宝是否处于"已投出"状态
