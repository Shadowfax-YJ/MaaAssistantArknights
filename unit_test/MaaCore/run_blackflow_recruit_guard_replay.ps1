...
if (!$OutputDirectory) { $OutputDirectory = Join-Path $repo 'build\blackflow-recruit-guard-replay' }
...
$generatorArgs = @((Join-Path $PSScriptRoot 'generate_blackflow_recruit_guard_replay.py'), (Join-Path $out 'replay.cpp'))
...
Push-Location $out
try {
    & cl.exe /nologo /std:c++20 /EHsc /MD /O2 /utf-8 @includes ('/I'+$dep+'\include\opencv4') ('/I'+$dep+'\include') ('/Fe'+$out+'\replay.exe') ($out+'\replay.cpp') /link ($dep+'\lib\opencv_world4.lib')
    if ($LASTEXITCODE) { exit $LASTEXITCODE }
