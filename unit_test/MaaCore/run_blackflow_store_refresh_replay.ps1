...
if (!$OutputDirectory) { $OutputDirectory = Join-Path $repo 'build\blackflow-store-refresh-replay' }
...
$generatorArgs = @((Join-Path $PSScriptRoot 'generate_blackflow_store_refresh_replay.py'), (Join-Path $out 'replay.cpp'))
